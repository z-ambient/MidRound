// MidRound — database schema + seed data
...
addColumn('teams', 'faceit_team_id', 'faceit_team_id TEXT');
addColumn('teams', 'faceit_team_name', 'faceit_team_name TEXT');
addColumn('teams', 'faceit_api_key', 'faceit_api_key TEXT');
addColumn('teams', 'faceit_last_sync', 'faceit_last_sync TEXT');
addColumn('matches', 'faceit_match_id', 'faceit_match_id TEXT');
addColumn('teams', 'faceit_roster', 'faceit_roster TEXT');
addColumn('opponents', 'faceit_team_id', 'faceit_team_id TEXT');
addColumn('opponent_players', 'faceit_player_id', 'faceit_player_id TEXT');
addColumn('opponent_players', 'faceit_stats', 'faceit_stats TEXT');
addColumn('matches', 'faceit_result', 'faceit_result TEXT');
addColumn('opponents', 'faceit_intel', 'faceit_intel TEXT');
addColumn('invites', 'email', 'email TEXT');
db.exec('CREATE UNIQUE INDEX IF NOT EXISTS idx_matches_faceit ON matches(team_id, faceit_match_id) WHERE faceit_match_id IS NOT NULL;');

const now = () => new Date().toISOString();
const J = JSON.stringify;

// roster slots: the players on the team, optionally controlled by a user account
...
function backfillTeamPlayers() {
  const insTp = db.prepare(`INSERT INTO team_players
    (team_id, name, game_role, is_starter, user_id, faceit_nickname, faceit_player_id, faceit_stats)
    VALUES (?,?,?,?,?,?,?,?)`);
  for (const team of db.prepare('SELECT id, faceit_roster FROM teams').all()) {
    if (db.prepare('SELECT COUNT(*) c FROM team_players WHERE team_id = ?').get(team.id).c) continue;
    const tms = db.prepare(`SELECT tm.user_id, tm.game_role, tm.is_starter, u.name
      FROM team_members tm JOIN users u ON u.id = tm.user_id
      WHERE tm.team_id = ? AND tm.game_role IS NOT NULL`).all(team.id);
    for (const tm of tms) insTp.run(team.id, tm.name, tm.game_role, tm.is_starter, tm.user_id, null, null, null);
    if (!tms.length) {
      let fr = []; try { fr = JSON.parse(team.faceit_roster) || []; } catch { /* none */ }
      for (const p of fr) {
        insTp.run(team.id, p.nickname || '?', null, 1, null, p.nickname || null, p.user_id || null,
...
function seedIfEmpty() {
  ...
  const insUser = db.prepare('INSERT INTO users (email, name, password_hash, created_at) VALUES (?,?,?,?)');
  const users = {};
  for (const [key, email, name] of [
    ...
    users[key] = insUser.run(email, name, hash, t).lastInsertRowid;
  }

  const orgId = db.prepare('INSERT INTO organizations (name, created_at) VALUES (?,?)').run('Northlight Gaming', t).lastInsertRowid;
  const teamId = db.prepare('INSERT INTO teams (org_id, name) VALUES (?,?)').run(orgId, 'Northlight Prime').lastInsertRowid;

  const insMember = db.prepare('INSERT INTO org_members (org_id, user_id, role) VALUES (?,?,?)');
  insMember.run(orgId, users.owner, 'owner');
  insMember.run(orgId, users.coach, 'edit');
  insMember.run(orgId, users.analyst, 'edit');
  insMember.run(orgId, users.igl, 'edit');
  insMember.run(orgId, users.p2, 'view');
  insMember.run(orgId, users.p3, 'view');
  insMember.run(orgId, users.p4, 'view');
  insMember.run(orgId, users.p5, 'view');

  const insTm = db.prepare('INSERT INTO team_members (team_id, user_id, game_role, is_starter) VALUES (?,?,?,?)');
  insTm.run(teamId, users.igl, 'IGL / Rifler', 1);
  insTm.run(teamId, users.p2, 'Entry', 1);
  insTm.run(teamId, users.p3, 'Support', 1);
  insTm.run(teamId, users.p4, 'Lurk', 1);
  insTm.run(teamId, users.p5, 'AWP', 1);

  for (const m of ['Mirage', 'Inferno', 'Nuke', 'Ancient', 'Anubis', 'Dust2', 'Train']) {
    db.prepare('INSERT INTO maps (name, active) VALUES (?,1)').run(m);
  }

  // ---- Opponents ----
  const insOpp = db.prepare(`INSERT INTO opponents
    (team_id, name, org_name, playstyle, map_pool, preferred_picks, preferred_bans, econ_notes, notes, created_at, updated_at)
    VALUES (?,?,?,?,?,?,?,?,?,?,?)`);
  const opp1 = insOpp.run(teamId, 'Ironclad Syndicate', 'Ironclad Org',
    ...
    t, t).lastInsertRowid;
  const opp2 = insOpp.run(teamId, 'Blue Harbor Esports', 'Blue Harbor',
  ...
  const insOp = db.prepare(`INSERT INTO opponent_players
    (opponent_id, name, role, positions, weapons, aggression, habits, weaknesses, notes) VALUES (?,?,?,?,?,?,?,?,?)`);
  const ir1 = insOp.run(opp1, 'fracture', 'AWP', 'Mirage: window, B short. Inferno: pit, arch.', 'AWP, Deagle on forces', 'passive',
    ...
    'Do not dry-peek mid on Mirage. Trade around his first shot.').lastInsertRowid;
  const ir2 = insOp.run(opp1, 'Kestrel', 'Entry / Connector', 'Mirage: connector, short. Ancient: mid.', 'Rifles', 'aggressive',
    ...
    'Punish underpass push with a late lurker.').lastInsertRowid;
  insOp.run(opp1, 'Bastion', 'IGL / Anchor', 'Mirage: B apps anchor. Inferno: B site.', 'Rifles, SMGs on anti-eco', 'passive',
  ...
  insOp.run(opp1, 'Havoc', 'Support', 'Mirage: A site / jungle.', 'Rifles', 'balanced',
    ...
    null);
  insOp.run(opp1, 'Marrow', 'Lurk', 'Mirage: palace / apps on T. CT: flexes.', 'Rifles', 'balanced',
  ...
  insOp.run(opp2, 'Meridian', 'AWP', 'Dust2: mid doors. Mirage: window / jungle.', 'AWP', 'aggressive',
  ...
  insT.run(opp1, null, null, null, null, null, 'Timeout',
  ...
  const matchId = db.prepare(`INSERT INTO matches
  ...
  const insPin = db.prepare('INSERT INTO match_pins (match_id, strategy_id, sort) VALUES (?,?,?)');
  [aSplit, fastB, midDefault, contactA, ctDefault, ctPistol].forEach((sid, i) => insPin.run(matchId, sid, i));

  const insNote = db.prepare('INSERT INTO match_notes (match_id, kind, text, sort) VALUES (?,?,?,?)');
  insNote.run(matchId, 'reminder', 'Round 2 after losing pistol: they ALWAYS force — buy armor + utility accordingly, do not anti-eco.', 0);
  insNote.run(matchId, 'reminder', 'Vector calls "MID OK" only after catwalk is checked — A Split fails without true mid control.', 1);
  insNote.run(matchId, 'reminder', 'Pillar: fracture (their AWP) starts window every gun round. Never dry-cross mid.', 2);
